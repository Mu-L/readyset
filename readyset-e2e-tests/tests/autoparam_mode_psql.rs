...
/// Where the previous statement went. Asked over the simple protocol, since the extended one
/// prepares the statement and Postgres itself has no `EXPLAIN LAST STATEMENT`.
async fn assert_last_target_was(client: &Client, expected: QueryDestination) {
    ...
    assert_eq!(destination, expected, "{:?}", row.get(1));
}
